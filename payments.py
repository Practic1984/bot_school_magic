...
PRICE = PRICE_BLOCK
prices = [LabeledPrice(label='Оплата услуги', amount=190000)]
...
def command_pay(message):
    print(message)
    bot.send_message(message.chat.id,
                     f"Оплата этапа стоимость {PRICE_BLOCK} руб.")
    bot.send_invoice(
                     message.chat.id,  #chat_id
                     'Оплата услуги', #title
                     'Белая психология', #description
                     'block', #invoice_payload
...
def got_payment(message):
    print(message)
    bot.send_message(message.chat.id, text = 'Поздравляю, сейчас вам пришлют файл')
